...
export function AIAssistant() {
  ...
  // Show survey after 2+ user messages
  useEffect(() => {
    if (!isOpen || surveyDismissed || surveySubmitted || userMessageCount < 2 || isTypingBot) return;
    const t = setTimeout(() => setSurveyVisible(true), 1200);
    return () => clearTimeout(t);
  }, [userMessageCount, isTypingBot, isOpen, surveyDismissed, surveySubmitted]);
  ...
  const handleSend = useCallback((text: string) => {
    ...
    setTimeout(() => {
      ...
      // Record to exchanges ref for session-level tracking
      exchangesRef.current.push({
        question: text,
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div ref={panelRef}
            ...
            className="fixed bottom-0 right-0 md:bottom-6 md:right-6 w-full md:w-[400px] h-[85vh] md:h-[640px] max-h-screen bg-background border border-border md:rounded-2xl shadow-2xl z-50 flex flex-col overflow-hidden"
          >
            {/* Header */}
            ...
            {/* Chat area */}
            <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50/50 dark:bg-transparent" ref={scrollRef}>
              {messages.length === 1 && !isTypingBot && (
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
                  <div>
                    <p className="text-[11px] font-bold text-muted-foreground mb-2 uppercase tracking-wider">Suggested Questions</p>
                    <div className="grid grid-cols-1 gap-2">
                      {suggestions.map((q, i) => (
                        <button key={i} onClick={() => handleSend(q)}
                          className="text-left text-sm bg-white dark:bg-slate-800 border border-border p-3 rounded-xl hover:border-primary/50 hover:bg-primary/5 transition-all text-foreground font-medium">
                          {q}
                        </button>
                      ))}
                    </div>
                  ...
                  <div>
                    <p className="text-[11px] font-bold text-muted-foreground mb-2 uppercase tracking-wider">Popular Topics</p>
                    <div className="flex flex-wrap gap-2">
                      {topics.map((t, i) => (
                        <button key={i} onClick={() => handleSend(t)}
                          className="text-xs bg-white dark:bg-slate-800 border border-border px-3 py-1.5 rounded-full hover:border-primary/50 hover:bg-primary/5 transition-colors font-medium">
                          {t}
                        </button>
                      ))}
                    </div>
            ...
            <div className="p-4 bg-background border-t border-border shrink-0">
              <form onSubmit={e => { e.preventDefault(); handleSend(input); }}
                ...
                <input
                  ...
                  className="flex-1 bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-full px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-[#0066FF]/30 focus:border-[#0066FF] transition-all"
                  disabled={isTypingBot}
                  onKeyDown={e => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); handleSend(input); } }}
                />
                <button type="submit" disabled={!input.trim() || isTypingBot}
                  className="w-10 h-10 rounded-full bg-[#0066FF] hover:bg-[#0052cc] disabled:opacity-40 text-white flex items-center justify-center shrink-0 transition-colors">
