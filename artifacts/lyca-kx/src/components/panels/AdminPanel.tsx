        ...
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-center gap-2 flex-wrap flex-1 min-w-0">
            <span className={`text-[9px] font-bold px-2 py-0.5 rounded-full shrink-0 ${
              session.queryType === "single"
                ? "bg-blue-100 text-blue-700 dark:bg-blue-900/40 dark:text-blue-300"
                : "bg-purple-100 text-purple-700 dark:bg-purple-900/40 dark:text-purple-300"
            }`}>
              {session.queryType === "single" ? "Single Query" : "Multi Query"}
            </span>
                  ...
                  {data && (
                    <>
                                      ...
                                      <table className="w-full text-sm">
                                        <thead className="bg-slate-50 dark:bg-slate-700/50">
                                          <tr>
                                            ...
                                            <th className="text-right p-3 text-xs font-bold text-muted-foreground">Users</th>
                                            <th className="text-left p-3 text-xs font-bold text-muted-foreground">Top Country</th>
                                            <th className="text-left p-3 text-xs font-bold text-muted-foreground">Last Seen</th>
                                          </tr>
                                        </thead>
                                          ...
                                          {data.topSearches.map((s, i) => {
                                            ...
                                            return (
                                              <tr key={s.term} className="hover:bg-slate-50 dark:hover:bg-slate-700/30">
                                                ...
                                                <td className="p-3 text-right text-muted-foreground">{s.uniqueUsers}</td>
                                                <td className="p-3 text-xs text-muted-foreground">{topCountry?.[0] ?? "—"}</td>
                                                <td className="p-3 text-xs text-muted-foreground">{new Date(s.lastSearched).toLocaleDateString()}</td>
                                              </tr>
                                            );
                                          })}
                                        </tbody>
                      ...
                      {tab === "ai" && (
                        <div className="space-y-4">
                          {/* Quick view */}
                          <QuickViewCard title="Quick View — Conversation Summary">
                            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-3">
                              {[
                                { label: "Total Conversations", value: data.totalConversations, color: "text-[#0066FF]" },
                                { label: "Single Query", value: data.conversationSessions.filter(s => s.queryType === "single").length, color: "text-blue-600" },
                                { label: "Multi Query", value: data.conversationSessions.filter(s => s.queryType === "multi").length, color: "text-purple-600" },
                                { label: "Agent Escalations", value: data.agentEscalations, color: "text-amber-600" },
                              ].map(item => (
                            ...
                            {data.conversationSessions.length === 0 ? (
                              ...
                              <div className="space-y-2">
                                {[...data.conversationSessions].reverse().slice(0, 3).map(session => (
                                  <div key={session.id} className="bg-white dark:bg-slate-700/50 border border-border rounded-lg px-3 py-2">
                                    <div className="flex items-center gap-2 mb-0.5">
                                      <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded-full ${session.queryType === "multi" ? "bg-purple-100 text-purple-700" : "bg-blue-100 text-blue-700"}`}>
                                        {session.queryType === "single" ? "Single" : "Multi"}
                                      </span>
                                      {session.wasEscalated && <span className="text-[9px] font-bold px-1.5 py-0.5 rounded-full bg-amber-100 text-amber-700">Agent</span>}
                                      <span className="text-[10px] text-muted-foreground">{session.country}</span>
                                      <span className="text-[10px] text-muted-foreground ml-auto">{new Date(session.startTime).toLocaleDateString()}</span>
                                    </div>
                                    <p className="text-xs font-medium truncate">{session.exchanges[0]?.question ?? "—"}</p>
                                  </div>
                                ))}
                              </div>
                            )}
