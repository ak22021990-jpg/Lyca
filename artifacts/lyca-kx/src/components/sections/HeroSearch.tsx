...
// Popular articles are deliberately DIFFERENT from trending topics above
...
export function HeroSearch() {
  ...
  const openArticle = (article: Article) => {
    ...
    setIsFocused(false);
    trackSearch(article.title, 1);
    if (!recentSearches.includes(article.title)) {
      setRecentSearches(prev => [article.title, ...prev].slice(0, 3));
    }
  };
  ...
  const handleTopicSelect = (label: string, articleId?: string) => {
    trackSearch(label, articleId ? 1 : 0);
    if (articleId) {
    ...
    if (!recentSearches.includes(label)) {
      setRecentSearches(prev => [label, ...prev].slice(0, 3));
    }
  };
  ...
  const startVoiceSearch = () => {
    ...
    recognition.onresult = (event) => {
      ...
      if (results.length > 0) trackSearch(transcript, results.length);
      setTimeout(() => setVoiceStatus(""), 3000);
    };

    recognition.onerror = (event) => {
      setVoiceStatus(`Error: ${event.error === "not-allowed" ? "Microphone access denied" : "Could not hear you, try again"}`);
    ...
    <>
      <section className="relative overflow-hidden bg-gradient-to-br from-[#0E1F5C] via-[#0E1F5C] to-[#0066FF] py-16 px-4 md:px-6" id="search">
        ...
        <div className="container mx-auto max-w-4xl relative z-10">
                        ...
                        <>
                          ...
                          {searchResults.map((article, idx) => (
                            <button key={article.id}
                              ...
                              onClick={() => { trackSearch(query, searchResults.length); openArticle(article); }}>
          ...
          {/* Recently viewed */}
          {recentSearches.length > 0 && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="hidden md:flex flex-col gap-2 mb-4">
              <h3 className="text-white/70 text-xs font-medium text-center">Recently Viewed:</h3>
              <div className="flex gap-2 justify-center flex-wrap">
                {recentSearches.map((s, i) => (
                  <span key={i} className="flex items-center gap-1.5 text-xs text-white/70 bg-white/10 px-3 py-1.5 rounded-full">
          ...
          {/* Popular articles — DIFFERENT from trending topics above */}
