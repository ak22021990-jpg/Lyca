...
import { ChevronRight, ChevronDown, ThumbsUp, ThumbsDown, HelpCircle, Clock, Calendar } from "lucide-react";
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-start">
            {faqs.map((faq, idx) => (
              <motion.div key={faq.id}
                ...
                className="bg-white dark:bg-slate-800 border border-border rounded-xl overflow-hidden shadow-sm">
                <button
                  onClick={() => setOpenFaq(openFaq === faq.id ? null : faq.id)}
                  className="w-full text-left px-5 py-4 flex items-start gap-3 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                  ...
                  <ChevronDown className={`w-4 h-4 text-muted-foreground shrink-0 mt-0.5 transition-transform duration-200 ${openFaq === faq.id ? "rotate-180" : ""}`} />
                </button>
                <AnimatePresence>
                  {openFaq === faq.id && (
                    <motion.div
                      ...
                      <div className="px-5 pb-5 pt-1 border-t border-border bg-slate-50/50 dark:bg-slate-800/50">
