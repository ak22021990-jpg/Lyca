...
export function trackSearch(term: string, resultsFound: number): void {
  if (!term.trim()) return;
  const events = readStore<SearchEvent>(KEYS.searches);
  events.push({
    id: uid(), term: term.trim().toLowerCase(), timestamp: Date.now(),
    sessionId: getSessionId(), country: getCountryFromTimezone(),
    timezone: Intl.DateTimeFormat().resolvedOptions().timeZone, resultsFound,
  });
...
export interface AnalyticsData {
  topSearches: { term: string; count: number; uniqueUsers: number; countries: Record<string, number>; lastSearched: number }[];
...
export function getAnalyticsData(): AnalyticsData {
  ...
  const sc: Record<string, { count: number; sessions: Set<string>; countries: Record<string, number>; last: number }> = {};
  ...
  const topSearches = Object.entries(sc)
    .map(([term, d]) => ({ term, count: d.count, uniqueUsers: d.sessions.size, countries: d.countries, lastSearched: d.last }))
  ...
  return {
    topSearches, countrySplit, categoryCounts, conversationSessions, articleFeedback, chatSurveys,
